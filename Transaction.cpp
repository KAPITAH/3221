...
void Transaction::data_gather(int months)
{
	...
	gathered_data.open("./Transactions/graphics.txt", ios::out);
	gathered_data << income << endl;
	gathered_data << expense << endl;
	for (map_it = data.begin(); map_it != data.end(); map_it++) {
		gathered_data <<Dictionaries::get_cat(map_it->first) + ":" + to_string(map_it->second / expense)<<endl;
	}
	gathered_data.close();
	system("python ./Transactions/plots.py");
}
...
Transaction::Transaction(User* current_user) {
	...
	if (answer == "1") {
		...
		cout << "\nWrite it manually in format MM/DD/YYYY" << endl;
		cin.ignore(1, '\n');
		getline(cin, answer);
		this->date = answer;
	}
